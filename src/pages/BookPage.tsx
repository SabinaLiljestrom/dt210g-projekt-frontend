...
export default function BookPage() {
  ...
  useEffect(() => {
    ...
    const run = async () => {
      ...
      setErr("");
      try {
        const b = await api(`/books/${id}`); // hämtar från backend
        const r = await api(`/reviews/${id}`); // hämtar från backend
        setBook(b);
        setReviews(r);
      } catch {
  ...
  if (!book) return <p style={{ padding: "2rem" }}>Ingen bok hittades.</p>;
  ...
  return (
    ...
    >
      ...
      >
        ...
        <div>
          ...
          {book.description ? (
            ...
            <p style={{ marginTop: 12, opacity: 0.8 }}>Ingen beskrivning.</p>
          )}
        </div>
      </div>
      ...
      <div style={{ maxWidth: 1000, margin: "0 auto" }}>
        ...
        {reviews.length === 0 ? (
          <p style={{ marginBottom: "1rem" }}>Inga recensioner ännu.</p>
        ) : (
          <div
